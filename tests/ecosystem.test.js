...
describe('2 - Verifica a configuração do ecosystem.config.yml', () => {
  ...
  it.skip('Será validado que o ecosystem tem a propriedade name', () => {
    expect(config).toHaveProperty('name');
  });

  it.skip('Será validado se o script a ser executado é o index.js', () => {
    expect(config).toHaveProperty('script');
    expect(config.script).toEqual('./index.js');
  });

  it.skip('Será validado que o modo de execução está configurado para cluster', () => {
    expect(config).toHaveProperty('exec_mode');
    expect(config.exec_mode).toEqual('cluster');
  });

  it.skip('Será validado que o número de instâncias está definido como 2.', () => {
    expect(config).toHaveProperty('instances');
    expect(config.instances).toEqual(2);
  });

  it.skip('Será validado que o modo watch está configurado para estar desativado.', () => {
    expect(config.watch).not.toBeTruthy();
  });

  it.skip('Será validado que a reiniciação de memória máxima esta configurada como 200M', () => {
